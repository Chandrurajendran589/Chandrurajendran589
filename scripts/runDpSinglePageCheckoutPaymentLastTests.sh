...
##############################################################################################################################
##
## Script to run Single Page Checkout performance payment below the DMR section tests.
##
##   Usage: runDpSinglePageCheckoutPaymentLastTests.sh [nsu] [run_count] [sleep_seconds]
##
...
##  At once users test
  runTest com.wsgc.dp.checkout.SinglePageCheckoutPaymentLastTest "SinglePageCheckoutPaymentLastTest_Run-${a}" -Ddp.baseUrl='https://www.perf.potterybarnteen.com' -Ddp.testVersion='atOnceUsers' -Ddp.sparc.atOnceUsers=1

## 	Constant Load test
  runTest com.wsgc.dp.checkout.SinglePageCheckoutPaymentLastTest "SinglePageCheckoutPaymentLastTest_Run-${a}" -Ddp.baseUrl='https://www.perf.potterybarnteen.com' -Ddp.testVersion='constantUsersPerSec' -Ddp.sparc.constantUsersPerSec=5 -Ddp.sparc.constantDuration=300 -Ddp.sparc.throttleRps=45 -Ddp.sparc.throttleInDuration=60 -Ddp.sparc.throttleHoldDuration=36000

##  Ramp Users Load Test
#  runTest com.wsgc.dp.checkout.SinglePageCheckoutPaymentLastTest "SinglePageCheckoutPaymentLastTest_Run-${a}" -Ddp.testVersion='rampUsers' -Ddp.sparc.rampUsers=500 -Ddp.sparc.rampDuration=180 -Ddp.sparc.throttleRps=50 -Ddp.sparc.throttleInDuration=60 -Ddp.sparc.throttleHoldDuration=3600
  done

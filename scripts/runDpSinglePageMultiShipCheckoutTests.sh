...
##############################################################################################################################
##
## Script to run Single Page Checkout performance tests.
##
##   Usage: runDpSinglePageCheckoutTests.sh [nsu] [run_count] [sleep_seconds]
##
...
***************************************************************************************************************************************************************
      ...
      ##  The easiest way to get data is to go to this page - https://{site you are trying to test}/admin/tests/find-products/available.html
      ##  The tests dont have assertions at this point so take a look at the results to make sure they are acceptable.
      ##  You can control the number of suborders created by changing the item quantity(dp.pip.itemQuantity) & suborder max(dp.sparc.multiship.suborder_max).
***************************************************************************************************************************************************************
"
...
for a in $(seq 1 ${RUN_COUNT}); do
  ...
  ##  At once users test
  runTest com.wsgc.dp.checkout.SinglePageMultiShipCheckoutTest "SinglePageMultiShipCheckoutTest_Run-${a}" -Ddp.baseUrl='https://www.perf.potterybarnteen.com' -Ddp.testVersion='atOnceUsers' -Ddp.sparc.atOnceUsers=1

  ## 	Constant Load test
  runTest com.wsgc.dp.checkout.SinglePageMultiShipCheckoutTest "SinglePageMultiShipCheckoutTest_Run-${a}" -Ddp.baseUrl='https://www.perf.potterybarnteen.com' -Ddp.testVersion='constantUsersPerSec' -Ddp.sparc.constantUsersPerSec=1 -Ddp.sparc.constantDuration=1 -Ddp.sparc.throttleRps=50 -Ddp.sparc.throttleInDuration=60 -Ddp.sparc.throttleHoldDuration=36000

  ##  Ramp Users Load Test
#  runTest com.wsgc.dp.checkout.SinglePageMultiShipCheckoutTest "SinglePageMultiShipCheckoutTest_Run-${a}" -Ddp.baseUrl='https://www.perf.potterybarnteen.com' -Ddp.testVersion='rampUsers' -Ddp.sparc.rampUsers=1 -Ddp.sparc.rampDuration=180 -Ddp.sparc.throttleRps=50 -Ddp.sparc.throttleInDuration=60 -Ddp.sparc.throttleHoldDuration=3600

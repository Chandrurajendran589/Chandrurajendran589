...
##############################################################################################################################
##
## Script to run promotion eligibility REST based pricing service performance tests
## The tests can be run multiple times
## By default, the tests wait 3 hours before they run
##
##   Usage: runPromotionEligibilityRestTests.sh [nsu] [run_count] [sleep_seconds]
##
...
for a in `seq 1 ${RUN_COUNT}`; do
  ...
  runTest com.wsgc.pricing.PromotionEligibilityRestTest "PromotionEligibilityRestTest_5_20_${a}"

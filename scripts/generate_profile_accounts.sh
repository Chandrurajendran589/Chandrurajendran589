...
echo email,encrypted_password,displayName,password,profileId > new_data.csv
for a in `seq 1 ${RECORD_COUNT}`; do
    PROFILE_ID=`uuidgen`
    echo ${EMAIL_PREFIX}${a}@wsgc.com,BCRYPT:${ENCRYPTED_PWD},${DISPLAY_NAME_PREFIX}${a},$PASSWORD,${PROFILE_ID} >> new_data.csv
done
exit 0
